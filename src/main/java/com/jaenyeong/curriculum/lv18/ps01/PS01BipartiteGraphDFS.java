package com.jaenyeong.curriculum.lv18.ps01;

import java.util.ArrayList;
import java.util.List;
import java.util.Scanner;

public class PS01BipartiteGraphDFS {
    /*
    이분 그래프 판별
    ...
    public static void main(String[] args) {
        ...
        final BipartiteGraphDFS bipartiteGraphDFS = new BipartiteGraphDFS(graph, n);
        bipartiteGraphDFS.execute();
    }
}
...
class BipartiteGraphDFS {
    ...
    public BipartiteGraphDFS(final List<List<Integer>> graph, final int n) {
        this.graph = graph;
    ...
    public void execute() {
        isBipartite(1, A_GROUP);

        System.out.println(isNotBipartite ? "No" : "Yes");
    }

    private void isBipartite(final int node, final char groupName) {
        // 방문
        visited[node] = true;
        // 그룹 설정
        ...
        for (int nextNode : graph.get(node)) {
            // 이미 방문한 인접 노드가 현재 노드와 그룹이 같은 경우
            if (visited[nextNode] && groups[nextNode] == groupName) {
                isNotBipartite = true;
                return;
            }

            if (!visited[nextNode]) {
                final char nextGroupName = (groupName == A_GROUP) ? B_GROUP : A_GROUP;
                isBipartite(nextNode, nextGroupName);
            }
        }
    }
}
